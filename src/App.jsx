...
export default function App() {
  ...
  const handleBarcodeDetected = (code) => {
    ...
    if (scannerTarget === "pos") {
      // POS mode: find product or offer to add new
      ...
      const found = products.find(p => p.barcode === code);
      if (found) showToast(`Already used by: ${found.name}`, "error");
      else { setNewProduct(p => ({ ...p, barcode: code })); showToast(`Barcode set: ${code} ✓`); }
    } else if (scannerTarget === "editProduct") {
      setEditFields(f => ({ ...f, barcode: code })); showToast(`Barcode set: ${code} ✓`);
    } else {
      // generic search
      const found = products.find(p => p.barcode === code);
      if (found) { setTab("Stock"); setSearch(found.name); showToast(`Found: ${found.name} ✓`); }
      else { setPosResult({ type: "new", barcode: code }); }
    }
  };
  ...
  const posAdjustStock = (product, delta) => {
    ...
    showToast(`${product.name}: stock ${delta > 0 ? "+" : ""}${delta} → ${Math.max(0, product.stock + delta)}`);
    setPosResult(null);
  };
  ...
  const posAddNewFromBarcode = (barcode) => {
    ...
    setTab("Add");
    setPosResult(null);
    showToast("Fill in product details");
  };
    ...
    <div style={{ fontFamily: "-apple-system,'SF Pro Display','Segoe UI',sans-serif", background: T.bg, minHeight: "100vh", color: T.text, maxWidth: 430, margin: "0 auto", position: "relative", overflowX: "hidden" }}>
              ...
              return (
                <div>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:20 }}>
                    <div style={{ background:`${T.success}20`, borderRadius:14, width:52, height:52, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, flexShrink:0 }}>
                      {p.image ? <img src={p.image} style={{ width:52, height:52, borderRadius:14, objectFit:"cover" }} /> : CAT_ICON[p.category] || "📦"}
                    </div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:800, fontSize:18 }}>{p.name}</div>
                      <div style={{ fontSize:12, color:T.muted }}>{p.company || p.brand} · {p.unit}</div>
                      <div style={{ fontSize:11, fontFamily:"monospace", color:T.muted, marginTop:2 }}>⌷ {posResult.barcode}</div>
                    </div>
                    <div style={{ background:`${stColor}20`, borderRadius:12, padding:"6px 12px", textAlign:"center" }}>
                      <div style={{ fontSize:22, fontWeight:900, color:stColor, lineHeight:1 }}>{p.stock}</div>
                      <div style={{ fontSize:9, color:stColor }}>in stock</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
                    <button onClick={() => posAdjustStock(p, -posQtyAdj)}
                      style={{ background:`${T.danger}18`, border:`1.5px solid ${T.danger}50`, borderRadius:14, padding:"14px", color:T.danger, fontWeight:800, fontSize:15, cursor:"pointer" }}>
                      − Sold / Used
                    </button>
                    <button onClick={() => posAdjustStock(p, +posQtyAdj)}
                      style={{ background:`${T.success}18`, border:`1.5px solid ${T.success}50`, borderRadius:14, padding:"14px", color:T.success, fontWeight:800, fontSize:15, cursor:"pointer" }}>
                      + Received
                    </button>
                  </div>
      ...
      {tab === "Settings" && (
        <div style={{ padding: "16px 16px 20px" }}>
          ...
          <div style={{ fontSize: 12, color: T.muted, marginBottom: 20 }}>Customise your inventory app</div>
          ...
          <div style={{ background: T.card, borderRadius: 18, border: `1px solid ${T.border}`, overflow: "hidden", marginBottom: 16 }}>
            ...
            {[
              ...
              <div key={key} style={{ padding: "12px 16px", borderBottom: `1px solid ${T.border}` }}>
                ...
                <input value={settings[key] || ""} onChange={e => setSetting(key, e.target.value)} placeholder={placeholder}
          ...
          <div style={{ background: T.card, borderRadius: 18, border: `1px solid ${T.border}`, overflow: "hidden", marginBottom: 16 }}>
            ...
            {[...new Set(products.map(p => p.company).filter(Boolean))].map(company => (
              <div key={company} style={{ padding: "12px 16px", borderBottom: `1px solid ${T.border}` }}>
                ...
                <input value={supplierPhones[company] || ""} onChange={e => setSupplierPhones(s => ({ ...s, [company]: e.target.value }))}
                  placeholder="WhatsApp number (e.g. 919876543210)" type="tel"
                  style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 10, padding: "9px 12px", color: T.text, fontSize: 13, outline: "none", width: "100%", boxSizing: "border-box" }} />
              </div>
            ))}
          </div>
