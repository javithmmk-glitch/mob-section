import React, { useState, useRef, useEffect, useCallback, useMemo, useTransition, startTransition, memo } from "react";
...
// ─── BARCODE SCANNER ─────────────────────────────────────────────────────────
// Wrapped in memo — only re-renders if props change
...
// ─── BARCODE SCANNER MEMO EXPORT ────────────────────────────────────────────
const BarcodeScannerMemo = memo(BarcodeScanner);

// ─── ERROR BOUNDARY ────────────────────────────────────────────────────────────
// Catches any crash and shows a friendly recovery screen instead of blank white
class ErrorBoundary extends React.Component {
  ...
  render() {
    ...
    return (
      <div style={{ fontFamily: "-apple-system,sans-serif", background: "#0d0d14", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, color: "#f0f0ff", textAlign: "center" }}>
        <div style={{ fontSize: 52, marginBottom: 16 }}>⚠️</div>
        <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 8 }}>Something went wrong</div>
        <div style={{ fontSize: 13, color: "#6b6b8a", marginBottom: 24, lineHeight: 1.6 }}>
          Your data is safe. Tap below to reload.
        </div>
        <button onClick={() => { this.setState({ error: null }); window.location.reload(); }}
          style={{ background: "linear-gradient(135deg,#7c6ef5,#a78bfa)", border: "none", borderRadius: 14, padding: "14px 28px", color: "#fff", fontWeight: 800, fontSize: 16, cursor: "pointer" }}>
          🔄 Reload App
        </button>
        <div style={{ marginTop: 16, fontSize: 11, color: "#444", fontFamily: "monospace" }}>
...
function AppInner() {
  ...
  const [scannerTarget, setScannerTarget] = useState("pos");
  // CRITICAL: scannerTarget as a REF so handleBarcodeDetected always reads the
  // latest value — useState alone causes stale closure bugs in async callbacks
  const scannerTargetRef = useRef("pos");

  // POS scan result modal — shows after any barcode scan
  const [posResult, setPosResult] = useState(null);
  // posResult shape: { type:"found"|"new", product?, barcode }
  const [posQtyAdj,  setPosQtyAdj]  = useState(1);
  ...
  // ── Settings draft states (must be at top level — no hooks in render) ────────
  const [draftStore, setDraftStore] = useState({ storeName: "", ownerName: "", storeAddress: "" });
  // ── Settings (persisted) ──────────────────────────────────────────────────
  ...
  const setSetting = useCallback((key, val) => setSettings(s => ({ ...s, [key]: val })), []);

  // Sync draftStore whenever settings tab opens or settings change
  useEffect(() => {
    setDraftStore({
  ...
  // ── Products ref — always current for async scanner callbacks ───────────────
  const productsRef = useRef(products);
  useEffect(() => { productsRef.current = products; }, [products]);

  // ── useTransition — tab switches never block the UI ─────────────────────────
  const [, startTabTransition] = useTransition();
  const switchTab = useCallback((t) => startTabTransition(() => setTab(t)), []);
  ...
  // ── Derived — ALL memoised so they only recompute when products changes ──────
  ...
  const dashStats = useMemo(() => {
    ...
    const avgMargin      = products.length
      ...
      : "0.0";
    return { totalCostValue, totalSellValue, totalProfit, avgMargin };
  }, [products]);
  const { totalCostValue, totalSellValue, totalProfit, avgMargin } = dashStats;

  const lowestStock = useMemo(
    () => [...needsReorder].sort((a, b) => a.stock - b.stock)[0],
    [needsReorder]
  );
  ...
  const handleBarcodeDetected = useCallback((rawCode) => {
    ...
    // READ FROM REF — not from state — avoids stale closure bug
    const target = scannerTargetRef.current;

    if (target === "pos" || target === "search") {
      if (found) {
        setPosQtyAdj(1);
        setPosResult({ type: "found", product: found, barcode: code });
      } else {
  ...
  const openScanner = useCallback((target) => {
    scannerTargetRef.current = target;  // sync ref immediately (no async delay)
    setScannerTarget(target);           // sync state for UI
    setShowScanner(true);
  }, []);
  ...
  const confirmSell = () => {
    ...
    setSellModal(null);
  }, [sellDate, sellTime, sellQty, sellNote, sellModal]);

  // After adding a product via barcode, show it in POS view
  const posAddNewFromBarcode = (barcode) => {
    setNewProduct(p => ({ ...p, barcode }));   // pre-fill barcode
    switchTab("Add");                           // go to Add Product tab
    setPosResult(null);                         // close POS modal
    showToast("Barcode pre-filled — fill in product details");
  };

  // After successful addProduct → show newly added product in POS modal
  // so user can scan again and instantly see the product they just added
  const showJustAdded = (addedProduct) => {
    setPosQtyAdj(1);
    setPosResult({ type: "found", product: addedProduct, barcode: addedProduct.barcode || "", justAdded: true });
  };
  ...
  const addProduct = useCallback(() => {
    ...
    // Show product in POS view — user can immediately scan to verify
    showJustAdded(addedProduct);
  }, [newProduct, products, supplierPhones, showToast]);
  ...
  }, [newCatName, categories, showToast]);
  ...
  const openReorderModal = useCallback((company, items) => {
  ...
  // ── Shared input style — memoised per theme so object refs stay stable ────────
  ...
  const inp = useCallback((extra = {}) => ({ ...inputBase, ...extra }), [inputBase]);
  ...
  const catHealth = useMemo(() =>
    categories.map(cat => {
      ...
      const low = items.filter(p => p.stock <= p.minLevel).length;
      return { cat, items, low, total: items.length };
    }).filter(Boolean),
    [products, categories]
  );

  // ─── PRODUCT IMAGE COMPONENT — memo prevents re-render on unrelated state ────
  const ProductThumb = useCallback(({ product, size = 52 }) => (
    product.image
  ...
  return (
    <div style={{ fontFamily: "-apple-system,'SF Pro Display','Segoe UI',sans-serif", background: T.bg, minHeight: "100vh", color: T.text, maxWidth: 430, margin: "0 auto", position: "relative", overflowX: "hidden" }}>
      ...
      {/* ── BARCODE SCANNER ─────────────────────────────────────────── */}
      {showScanner && <BarcodeScannerMemo onDetected={handleBarcodeDetected} onClose={() => setShowScanner(false)} T={T} />}
      ...
      {tab === "Settings" && (
        <div style={{ padding: "16px 16px 20px" }}>
          ...
          {(()=>{
            ...
            const saveStore = () => { setSetting("storeName", draftStore.storeName); setSetting("ownerName", draftStore.ownerName); setSetting("storeAddress", draftStore.storeAddress); showToast("Store info saved ✓"); };
            return (
          ...
          <div style={{ background: T.card, borderRadius: 18, border: `1px solid ${T.border}`, overflow: "hidden", marginBottom: 16 }}>
            ...
            {[...new Set(products.map(p => p.company).filter(Boolean))].map(company => (
              <div key={company} style={{ padding: "12px 16px", borderBottom: `1px solid ${T.border}` }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    value={supplierPhones[company] || ""}
                    onChange={e => setSupplierPhones(s => ({ ...s, [company]: e.target.value }))}
                  ...
                  <button onClick={() => showToast(`${company} saved ✓`)}
                    style={{ background: T.btnGrad, border: "none", borderRadius: 10, padding: "9px 14px", color: "#fff", fontWeight: 700, cursor: "pointer", fontSize: 13, flexShrink: 0 }}>
                    Save
                  </button>
                </div>
              </div>
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: T.navBg, borderTop: `1px solid ${T.border}`, padding: "10px 0 24px", display: "flex", justifyContent: "space-around", alignItems: "center", zIndex: 500 }}>
        {[
          ...
          return (
            <button key={key} onClick={() => switchTab(key)} style={{ background:"none", border:"none", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:4, padding:"4px 10px", color: isAdd ? "transparent" : active ? T.accent : T.muted }}>
              {icon}
